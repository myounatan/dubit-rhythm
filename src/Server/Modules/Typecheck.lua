...
-- wraps game enum in a pcall
local function testGameEnum(gameenum)
    return
end

-- move to gamecheck
function t.GameEnum(gameenum)
    --print("t.GameEnum(", gameenum, ")")
    return function(val)
        local success =
            pcall(
            function()
                --print(gameenum[val])
                local testvar = gameenum[val]
            end
        )
        return success
    end
